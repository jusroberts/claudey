...
- [ ] Add FCM to the Android app (google-services, `FirebaseMessagingService`)
- [ ] App registers its FCM token with the server: `POST /api/devices {token}`
- [ ] Server `Push` module sends **high-priority data messages** via FCM HTTP v1
      (service-account JSON via env var)
- [ ] Data messages either carry a ready-to-show notification payload or a
...
- [ ] Add `ecto_sqlite3` (SQLite file on the homelab box — no Postgres needed)
- [ ] Initial tables: `devices` (push tokens), `runs`, `events`,
      `coach_suggestions`, `finance_baselines`, `report_log`

### 0.3 Server scheduling
- [ ] Add Quantum (cron-style scheduler, no DB dependency) to the supervision
...
- [ ] Podman: add `actual-http-api` container (pin to the same 26.x line as
      the Actual server; env: `ACTUAL_SERVER_URL`, `ACTUAL_SERVER_PASSWORD`,
      sync ID, generated `API_KEY`)
- [ ] Phoenix `Finance.ActualClient` (Req/Finch): accounts+balances,
      transactions by date range, budget months. Amounts are integer cents.
- [ ] **Agent tools** (server-side, registered in `Tools.Registry`):
  - [ ] `get_net_worth` — sum of all on/off-budget account balances
  - [ ] `get_account_balance` — fuzzy-match account name ("credit card")
  - [ ] `get_spending_summary` — category/payee totals for a period
- [ ] **Anomaly report job** (Quantum, e.g. weekly Sunday evening + optional
      daily quick-check):
  - [ ] Pull recent transactions, aggregate per category and payee
  - [ ] Compare against trailing 3–6 month baseline stored in
        `finance_baselines` (median + MAD; flag |z| above threshold, new
        payees over a floor amount, duplicate-charge heuristic)
  - [ ] Compose report — feed the raw flags through the LLM for a readable
        2–3 sentence summary, with the numeric table as expandable detail
  - [ ] Push to phone via Phase 0 FCM; log to `report_log` so reports are
...
- [ ] `RunReminderWorker`: record outcome (fired / skipped-because-ran /
      skipped-by-probability / error) with timestamp in DataStore
- [ ] Settings → diagnostics: show last outcome per alarm, channel-enabled
      check (`areNotificationsEnabled` + per-channel), exact-alarm permission
      state, battery-exemption state — all with fix-it buttons
- [ ] Fallback to `setAndAllowWhileIdle()` when `canScheduleExactAlarms()` is
      false instead of silently not firing
- [ ] Revisit `isLikelyRunDay()` gate: probability threshold 0.35 suppresses
...
- [ ] Quantum job at 18:00 sends FCM data message `wake: run_reminder`
- [ ] App's FCM service enqueues `RunReminderWorker` (expedited) — Health
      Connect check still happens on-device where the data lives
- [ ] Keep the local 6pm alarm as belt-and-braces; dedupe by notification ID
- [ ] Once Phase 3 syncs runs to the server, the server can skip the wake
...
- [ ] **Garmin API (primary, server-side)**: OAuth setup, pull/webhook
      activities with HR series, elevation, pace, training effect into `runs`.
      No phone dependency; richest data.
- [ ] **Health Connect (secondary, client-side)**: periodic WorkManager job
...
- [ ] Dedupe by start-time window + duration

### 3.2 Events & plans
- [ ] `events` table: name, date, distance, goal. CRUD via
      `POST/GET /api/events` plus agent tools (`add_event`, `list_events`) so
      "I signed up for a half marathon on Oct 4" works in chat
- [ ] Coach engine (server): weekly Quantum job (e.g. Sunday) builds context —
...
- [ ] Agent tools: `get_week_plan`, `explain_suggestion`, "replan my week"

### 3.3 Coach UI (app)
- [ ] New `COACH` route: week calendar, each day showing suggested workout vs
      what actually happened (✓ matched / partial / missed), event countdown
      banner
- [ ] Day detail: suggestion rationale + actual run stats (pace, HR, elevation)
...
- [ ] `POST /api/tmux/sessions {name, cwd, run_claude: bool}` →
      `tmux new-session -d -s name -c cwd` (+ optional `send-keys 'claude' Enter`)
- [ ] **Path safety**: `Path.expand/1` + verify the real path
      (symlink-resolved) is under the server user's `$HOME`; reject otherwise.
      Validate session name (`[a-zA-Z0-9_-]+`).
- [ ] `GET /api/fs/dirs?path=` — list subdirectories, same home-only
      constraint, for the picker
- [ ] App: "＋" on TmuxSessionsScreen → dialog with directory browser
...
- [ ] Fix input fidelity first: use `tmux send-keys -l` for literal text and
      send `Enter` as a named key (current code can't cleanly send
      Enter/arrows, which is why answering prompts is painful)
- [ ] Server-side prompt detection in `TmuxChannel`'s poll: recognize Claude
...
- [ ] App: render detected options as tappable buttons above the input; tap
      sends the option key + Enter in one shot
- [ ] Quick-key row on the terminal screen regardless: Enter, Esc, ↑ ↓, Tab,
...
- [ ] **Unified daily digest**: one morning push assembled server-side
      (weather/run brief + commute + finance flags + today's coach suggestion)
      replacing N separate notifications
- [ ] **Memory**: small SQLite-backed notes table + `remember`/`recall` agent
